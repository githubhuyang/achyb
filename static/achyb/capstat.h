...
STATISTIC(CPUnResolv,
          "Critical Function Pointer Unable to Resolve, Collect Pass");
...
STATISTIC(CFuncUsedByStaticAssign,
          "Critical Functions used by static assignment");
STATISTIC(MatchCallCriticalFuncPtr,
          "# of times indirect call site matched with critical functions");
STATISTIC(
    UnMatchCallCriticalFuncPtr,
    "# of times indirect call site failed to match with critical functions");
STATISTIC(CapChkInFPTR,
          "found capability check inside call using function ptr\n");
STATISTIC(CritFuncSkip, "number of critical function skipped(uniq)\n");

#ifdef CUSTOM_STATISTICS
void achyb::dump_statistics() {
  errs() << "------------STATISTICS---------------\n";
