...
// Yang: compact several db files into one db, and sync to file system
func (db *DB) Merge(odb_lst []*DB) {
	if odb_lst == nil || len(odb_lst) == 0 {
		return
	}

	for _, odb := range odb_lst {
		for k, r := range odb.Records {
			db.Save(k, r.Val, r.Seq)
			// db.Records[k] = r
		}
	}
