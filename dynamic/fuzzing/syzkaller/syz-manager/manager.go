...
func RunManager(cfg *mgrconfig.Config, target *prog.Target, sysTarget *targets.Target, syscalls []int) {
	...
	log.Logf(0, "ACHyb: loading corpus...")
	mgr.corpusDB, err = db.Open(filepath.Join(cfg.Workdir, "corpus.db"))
	if err != nil {
		log.Fatalf("ACHyb: failed to open corpus database: %v", err)
	}

	// Yang: open more dbs, merge them and sync to fs
	var odb_lst []*db.DB
	for _, odb_file := range mgr.cfg.ODBFileLst {
		odb, err := db.Open(filepath.Join(cfg.Workdir, "seeds", odb_file))
		if err != nil {
			// log.Logf(0, "failed to open outside corpus database: %v", odb_file)
			log.Fatalf("failed to open outside corpus database: %v", err)
			continue
		}
		odb_lst = append(odb_lst, odb)
	}

	log.Logf(0, "ACHyb: merge outside dbs with main db")
	mgr.corpusDB.Merge(odb_lst)

	log.Logf(0, "ACHyb: sync merged db into corpus.db")
	mgr.corpusDB.Flush()

	log.Logf(0, "ACHyb: reloading corpus...")
	mgr.corpusDB, err = db.Open(filepath.Join(cfg.Workdir, "corpus.db"))
	if err != nil {
		log.Fatalf("ACHyb: failed to reopen corpus database: %v", err)
	}
